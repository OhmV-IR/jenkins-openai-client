...
public class OpenAIClient extends ModelClient<OpenAIModelSettings, OpenAIClientSettings> {
    ...
    protected List<ModelOutput> takeStepImpl(
        ...
        for (ModelInput modelInput : turnInputs) {
            switch (modelInput) {
                ...
                case InputSkillContent value ->
                    append(instructions, value.getSkill().getSkillText());
                ...
                case ThinkingLevelContent value ->
                    reasoningEffort = switch (value.getThinkingLevel()) {
                        case LOW -> "low";
                        case MEDIUM -> "medium";
        ...
        ResponseCreateParams.Builder builder = ResponseCreateParams.builder()
                ...
                .putAdditionalBodyProperty(
                        "input", JsonValue.from(input.isEmpty() ? List.of(message("user", "")) : input));
        ...
        if (reasoningEffort != null)
            builder.putAdditionalBodyProperty("reasoning", JsonValue.from(Map.of("effort", reasoningEffort)));
        ...
        try {
                ...
                if (item.isMessage()) {
                    for (var content : item.asMessage().content()) {
                        if (content.isOutputText())
                            outputs.add(
                                    new OutputTextContent(content.asOutputText().text()));
                        else if (content.isRefusal())
                            outputs.add(
                                    new OutputTextContent(content.asRefusal().refusal()));
                    }
                } else if (item.isFunctionCall()) {
                    var call = item.asFunctionCall();
                    JsonObject arguments =
                            JsonParser.parseString(call.arguments()).getAsJsonObject();
            ...
            response.incompleteDetails()
                    .ifPresent(ignored -> outputs.add(new FinishReasonContent(ModelFinishReason.TOKEN_CAP)));
            state.hadToolCalls = toolCall;
            return filterOutputs(request, outputs);
        } catch (OpenAIServiceException e) {
            state.hadToolCalls = false;
            throw new IllegalStateException(
                    "OpenAI API request failed with HTTP " + e.statusCode() + ": " + e.body(), e);
        } finally {
            client.close();
        }
    }
    ...
    private static Map<String, Object> filePart(InputFileContent value) {
        ...
        return Map.of(
                ...
                "filename",
                value.getFileId() == null ? "attachment" : value.getFileId());
    }
    ...
    private static Map<String, Object> tool(InputToolContent value) {
        ...
        for (ToolArgumentDescription argument : value.getTool().getArguments()) {
            String type = argument.getType() == String.class
                    ? "string"
                    : argument.getType() == boolean.class
                            ? "boolean"
                            : Number.class.isAssignableFrom(argument.getType())
                                            || argument.getType().isPrimitive()
                                    ? "number"
                                    : "object";
        ...
        return Map.of(
                ...
                "function",
                "name",
                value.getTool().getName(),
                "description",
                value.getTool().getDescription(),
                "parameters",
